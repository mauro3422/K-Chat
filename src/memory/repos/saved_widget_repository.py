...
class SavedWidgetRepository(_BaseRepository):
    ...
    async def delete_by_session(self, session_id: str, cursor: Any = None) -> None:
        ...
        if cursor is not None:
            await cursor.execute("DELETE FROM widget_versions WHERE widget_id IN (SELECT widget_id FROM saved_widgets WHERE session_id = ?)", (session_id,))
            await cursor.execute("DELETE FROM saved_widgets WHERE session_id = ?", (session_id,))
        else:
            async with self._transaction() as conn:
                await conn.execute("DELETE FROM widget_versions WHERE widget_id IN (SELECT widget_id FROM saved_widgets WHERE session_id = ?)", (session_id,))
                await conn.execute("DELETE FROM saved_widgets WHERE session_id = ?", (session_id,))
