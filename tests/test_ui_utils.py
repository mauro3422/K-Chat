...
def test_render_msg_with_phases_user():
    """Test rendering of a user message."""
    html_out = _render_msg_with_phases("user", "Hello <world>", None, [], ts=123)
...
def test_render_msg_with_phases_assistant_legacy():
    """Test rendering of a legacy assistant message (no phases, only content/reasoning)."""
    matched_tools = [
        ("web_search", "query1", "ok", 15, 1)
    ]
    html_out = _render_msg_with_phases(
        "assistant",
        "Final content",
        "Thinking deep...",
        matched_tools,
...
def test_render_msg_with_phases_assistant_sequential():
    ...
    assert idx_r1 < idx_t1 < idx_c1 < idx_r2 < idx_t2 < idx_c2

def test_render_msg_with_phases_fallback_empty_phases():
