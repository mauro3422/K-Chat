...
@pytest.mark.anyio
@patch("src.llm.retry.asyncio.sleep")
async def test_execute_with_retry_success_on_first_try(mock_sleep):
    mock_fn = MagicMock(return_value="success")

    result = await execute_with_retry(mock_fn, "test-model")
...
@pytest.mark.anyio
@patch("src.llm.retry.asyncio.sleep")
async def test_execute_with_retry_retries_on_rate_limit(mock_sleep):
...
@pytest.mark.anyio
@patch("src.llm.retry.asyncio.sleep")
async def test_execute_with_retry_uses_exponential_backoff(mock_sleep):
    class RateLimitError(Exception):
        status_code = 429

    mock_fn = MagicMock(side_effect=[RateLimitError("Rate limit"), "success"])

    await execute_with_retry(mock_fn, "test-model")
...
@pytest.mark.anyio
@patch("src.llm.retry.asyncio.sleep")
async def test_execute_with_retry_raises_after_max_retries(mock_sleep):
    class RateLimitError(Exception):
        status_code = 429
...
@pytest.mark.anyio
async def test_is_rate_limit_error_detects_rate_limit_error():
...
@pytest.mark.anyio
async def test_is_rate_limit_error_detects_status_code_429():
...
@pytest.mark.anyio
async def test_is_rate_limit_error_detects_text_patterns():
