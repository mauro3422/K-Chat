...
async def test_jsonl_handler_emit():
    ...
    config = MagicMock()
    config.kairos_log_dir = "/tmp/test_logs"

    with patch.object(file_logger.JsonlHandler, "_ensure_dirs"):
        with patch("builtins.open", mock_open()) as mock_file:
            handler = file_logger.JsonlHandler(module="test", config=config)
...
async def test_ensure_dirs():
    ...
    with patch("pathlib.Path.mkdir") as mock_mkdir:
        file_logger.JsonlHandler(module="test", config=config)
...
async def test_install_jsonl_handler():
    ...
    with patch("web.services.file_logger.JsonlHandler") as mock_handler_class:
        ...
        with patch("logging.getLogger") as mock_get_logger:
            ...
            mock_handler_class.assert_called_once_with("test_module", config=None)
...
async def test_install_jsonl_handler_default_module():
    ...
    with patch("web.services.file_logger.JsonlHandler") as mock_handler_class:
        ...
        with patch("logging.getLogger") as mock_get_logger:
            ...
            assert result == mock_handler
            mock_handler_class.assert_called_once_with("app", config=None)
            mock_logger.addHandler.assert_called_once_with(mock_handler)
...
async def test_root_logger_has_add_handler():
    ...
    assert hasattr(file_logger.JsonlHandler, "_ensure_dirs")
