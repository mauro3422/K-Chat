...
class TestRunCode:
    ...
    async def test_simple_python_execution(self):
        ...
        result = await run_code.run(code="print('Hello, World!')")
    ...
    async def test_syntax_error_auto_fix(self):
        ...
        result = await run_code.run(code="print 'hello'")
        data = json.loads(result)
    ...
    async def test_blocked_module_os(self):
        ...
        code = "import os\nprint(os.getcwd())"
        result = await run_code.run(code=code)
        data = json.loads(result)
    ...
    async def test_timeout(self):
        ...
        code = "import time\ntime.sleep(100)"
        result = await run_code.run(code=code, timeout=2)
        data = json.loads(result)
        assert data["status"] == "error"
