...
class TestReadFile:
    ...
    async def test_reads_file(self, tmp_path):
        ...
        result = await read_file.run(path=str(path))
    ...
    async def test_pagination_default_caps_at_250_lines(self, tmp_path):
        path = tmp_path / "test.txt"
        path.write_text("\n".join(f"line {i}" for i in range(300)))
        result = await read_file.run(path=str(path))
        assert "Displayed range: 1-250" in result

    async def test_start_end_line(self, tmp_path):
        path = tmp_path / "test.txt"
        path.write_text("a\nb\nc\nd\ne\n")
        result = await read_file.run(path=str(path), start_line=2, end_line=4)
        assert "Displayed range: 2-4" in result
        assert "2: b" in result
        assert "4: d" in result

    async def test_path_traversal_rejected(self, tmp_path):
        result = await read_file.run(path=str(tmp_path / "../../../etc/passwd"))
        assert result.startswith("[ERROR]")
...
class TestWriteFile:
    ...
    async def test_writes_file(self, tmp_path):
        path = tmp_path / "test.txt"
        result = await write_file.run(path=str(path), content="hello")
        assert path.read_text() == "hello"
        assert "[OK]" in result

    async def test_creates_directories(self, tmp_path):
        path = tmp_path / "a" / "b" / "test.txt"
        result = await write_file.run(path=str(path), content="deep")
        assert path.read_text() == "deep"
        assert "[OK]" in result

    async def test_path_traversal_rejected(self):
        result = await write_file.run(path="/etc/pwned", content="x")
        assert result.startswith("[ERROR]")

    async def test_overwrites_existing_file(self, tmp_path):
        path = tmp_path / "overwrite.txt"
        path.write_text("old content")
        result = await write_file.run(path=str(path), content="new content")
        assert path.read_text() == "new content"
        assert "[OK]" in result

    async def test_writes_empty_content(self, tmp_path):
        path = tmp_path / "empty.txt"
        result = await write_file.run(path=str(path), content="")
...
class TestEditFile:
    ...
    async def test_replaces_line_range(self, tmp_path):
        path = tmp_path / "test.txt"
    ...
    async def test_inserts_before_line(self, tmp_path):
        path = tmp_path / "test.txt"
    ...
    async def test_deletes_line_range(self, tmp_path):
        path = tmp_path / "test.txt"
        path.write_text("keep\ndelete\nkeep\n")
        result = edit_file.run(path=str(path), start_line=2, end_line=2)
    ...
    async def test_no_operation_specified(self, tmp_path):
        path = tmp_path / "test.txt"
