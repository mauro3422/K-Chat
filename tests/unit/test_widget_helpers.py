...
def test_sanitize_id_allows_alphanumeric():
...
def test_sanitize_id_preserves_hyphen():
    assert sanitize_widget_id("hello-world") == "hello-world"


def test_sanitize_id_allows_hyphen_and_underscore():
    assert sanitize_widget_id("my-widget_v2") == "my-widget_v2"


def test_sanitize_id_strips_whitespace():
    assert sanitize_widget_id("  foo  ") == "foo"


def test_sanitize_id_only_special_chars():
    assert sanitize_widget_id("!@#$%") == ""


def test_sanitize_id_mixed_case():
    assert sanitize_widget_id("MyCoolWidget") == "mycoolwidget"


def test_validate_args_missing_session_id():
    result = validate_widget_args(None, "my_widget")
    assert isinstance(result, str)
    assert "session_id" in result


def test_validate_args_invalid_widget_id():
...
def test_validate_args_empty_session_id():
