...
async def test_read_skill_success():
    ...
    res = await read_skill_run("html-widgets", _skill_registry=_skill_registry)
...
async def test_read_skill_not_found():
    ...
    res = await read_skill_run("non-existent-skill-xyz", _skill_registry=_skill_registry)
    assert "[ERROR]" in res
...
async def test_read_skill_sanitization():
    ...
    # Directory traversal
    res = await read_skill_run("../../etc/passwd", _skill_registry=_skill_registry)
    # Sanitized to 'etcpasswd', which won't exist
    assert "[ERROR]" in res
    assert "etcpasswd" in res

    # Empty name
    res2 = await read_skill_run("...///...", _skill_registry=_skill_registry)
