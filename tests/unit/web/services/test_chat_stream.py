...
def _patch_infrastructure():
    """Prevent real repos / service construction inside ``generate()``."""
    with (
        patch("web.services.chat_stream.get_repos") as mock_get_repos,
        patch("web.services.chat_stream.HistoryService") as mock_hs,
        patch("web.services.chat_stream.LLMService") as mock_llm,
        patch("web.services.chat_stream.ToolExecutionService") as mock_tes,
        patch("web.services.chat_stream.TelemetryService") as mock_ts,
    ):
        mock_get_repos.return_value = MagicMock()
        mock_hs.return_value = MagicMock()
        mock_llm.return_value = MagicMock()
        mock_tes.return_value = MagicMock()
...
class TestNormalStreaming:
    ...
    @pytest.mark.asyncio
    async def test_content_tokens(self, deps, background_tasks):
        tokens = ["Hello", " ", "world", "!"]
    ...
    @pytest.mark.asyncio
    async def test_reasoning_and_content(self, deps, background_tasks):
...
class TestToolCalls:
    ...
    async def test_tool_call_forwarded(self, deps, background_tasks):
    ...
    async def test_content_around_tool_call(self, deps, background_tasks):
        deps.chat_stream_fn = _make_stream([
    ...
    async def test_tool_call_resets_state(self, deps, background_tasks):
...
class TestMemoryEvents:
    ...
    async def test_memory_forwarded(self, deps, background_tasks):
...
class TestHeartbeat:
    ...
    async def test_heartbeat_forwarded(self, deps, background_tasks):
        deps.chat_stream_fn = _make_stream([
            ("heartbeat", ""),
            ("content", "data"),
            ("heartbeat", ""),
    ...
    async def test_heartbeat_not_appended_to_state(self, deps, background_tasks):
...
class TestLoopDetection:
    ...
    async def test_loop_error_yielded(self, deps, background_tasks):
    ...
    async def test_loop_check_called_for_content(self, deps, background_tasks):
    ...
    async def test_non_content_not_checked(self, deps, background_tasks):
...
class TestLoopRecovery:
    ...
    async def test_recovery_content_yielded(self, deps, background_tasks):
        deps.loop_detector.check.return_value = "Loop detectado"
    ...
    async def test_recovery_receives_context(self, deps, background_tasks):
        deps.loop_detector.check.return_value = "Loop detectado"
        deps.retry_handler.can_retry = True
...
class TestLoopRecoveryFailure:
    ...
    async def test_error_when_recovery_empty(self, deps, background_tasks):
        deps.loop_detector.check.return_value = "Loop detectado"
        deps.retry_handler.can_retry = True
        deps.retry_handler.attempt_recovery = _make_stream([])
        deps.chat_stream_fn = _make_stream([("content", "aaaa")])
    ...
    async def test_error_when_recovery_raises(self, deps, background_tasks):
        deps.loop_detector.check.return_value = "Loop detectado"
        deps.retry_handler.can_retry = True
    ...
    async def test_recovery_not_attempted_when_cannot_retry(self, deps, background_tasks):
        deps.loop_detector.check.return_value = "Loop detectado"
...
class TestClientDisconnect:
    ...
    async def test_generator_exit_returns_cleanly(self, deps, background_tasks):
    ...
    async def test_vectorization_scheduled_on_disconnect(self, deps, background_tasks):
        deps.chat_stream_fn = _make_stream([("content", "hello")])
...
class TestEmptyResponse:
    ...
    async def test_empty_stream_yields_error(self, deps, background_tasks):
        gen = build_stream_generator(
            session_id="s1", message="hi", history=[], model="gpt4",
            background_tasks=background_tasks, deps=deps,
        )
...
class TestStreamError:
    ...
    async def test_error_classified_and_yielded(self, deps, background_tasks):
        deps.retry_handler = None
    ...
    async def test_retry_on_exception_with_output(self, deps, background_tasks):
    ...
    async def test_retry_not_attempted_without_output(self, deps, background_tasks):
...
class TestPeriodicSave:
    ...
    async def test_periodic_save_triggered_by_time(self, deps, background_tasks):
        deps.chat_stream_fn = _make_stream([
            ("content", "a"),
            ("content", "b"),
            ("content", "c"),
    ...
    async def test_periodic_save_not_called_before_interval(self, deps, background_tasks):
        deps.chat_stream_fn = _make_stream([
            ("content", "a"),
            ("content", "b"),
        ])
    ...
    async def test_periodic_save_not_called_without_content(self, deps, background_tasks):
...
class TestFinalSave:
    ...
    async def test_final_save_called(self, deps, background_tasks):
    ...
    async def test_rename_scheduled_after_save(self, deps, background_tasks):
        deps.chat_stream_fn = _make_stream([("content", "done")])
    ...
    async def test_final_save_not_called_on_empty(self, deps, background_tasks):
...
class TestInterruptionSave:
    ...
    async def test_saves_partial_on_exception(self, deps, background_tasks):
        deps.retry_handler = None
        deps.save_fn = AsyncMock()
    ...
    async def test_not_saved_when_no_output(self, deps, background_tasks):
    ...
    async def test_not_saved_when_already_persisted(self, deps, background_tasks):
    ...
    async def test_vectorization_always_scheduled(self, deps, background_tasks):
