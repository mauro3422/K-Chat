...
@pytest.mark.anyio
async def test_rate_limit_case_insensitive():
    etype, _ = classify_error("RATE LIMIT exceeded")
    assert etype == "rate_limit"


@pytest.mark.anyio
async def test_rate_limit_429_code():
    etype, _ = classify_error("HTTP 429 Too Many Requests")
    assert etype == "rate_limit"


@pytest.mark.anyio
async def test_rate_limit_uses_reset_header_hint():
    err = _RateLimitError(
...
@pytest.mark.anyio
async def test_rate_limit_prefers_longest_reset_hint():
    err = _RateLimitError(
        "HTTP 429 Too Many Requests",
        {
...
@pytest.mark.anyio
async def test_network_keyword():
...
@pytest.mark.anyio
async def test_priority_rate_limit_over_connection():
    etype, _ = classify_error("rate limit and connection error")
    assert etype == "rate_limit"


@pytest.mark.anyio
async def test_priority_timeout_over_model():
