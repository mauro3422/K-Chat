...
class TestExecuteCommand:
    @pytest.mark.anyio
    async def test_definition_exists(self):
        assert execute_command.DEFINITION["function"]["name"] == "execute_command"

    @pytest.mark.anyio
    async def test_definition_has_command_param(self):
    ...
    @pytest.mark.anyio
    async def test_error_on_empty_command(self):
        result = execute_command.run(command="  ")
        assert "[ERROR]" in result

    @pytest.mark.anyio
    async def test_error_on_nonexistent_command(self, tmp_path):
...
class TestListFiles:
    ...
    @pytest.mark.anyio
    async def test_definition_has_pattern_param(self):
    ...
    async def test_glob_pattern(self, tmp_path):
        ...
        result = await list_files.run(path=str(tmp_path), depth=0, pattern="*.py")
        assert "alpha.py" in result
        assert "beta.txt" not in result

    @pytest.mark.anyio
    async def test_empty_directory(self, tmp_path):
        result = await list_files.run(path=str(tmp_path), depth=0)
...
class TestSearchFiles:
    ...
    @pytest.mark.anyio
    async def test_finds_content_in_files(self, tmp_path):
        (tmp_path / "test.txt").write_text("hello world\nfoo bar\n")
        result = await search_files.run(pattern="hello", path=str(tmp_path))
        assert "hello" in result
    ...
    async def test_no_match_returns_empty(self, tmp_path):
        (tmp_path / "test.txt").write_text("hello world\n")
        result = await search_files.run(pattern="zzz_nonexistent", path=str(tmp_path))
        assert "Sin coincidencias" in result

    @pytest.mark.anyio
    async def test_case_sensitivity(self, tmp_path):
        (tmp_path / "test.txt").write_text("Hello World\n")
        insensitive = await search_files.run(pattern="hello", path=str(tmp_path), case_sensitive=False)
        sensitive = await search_files.run(pattern="hello", path=str(tmp_path), case_sensitive=True)
        assert "Hello" in insensitive
        assert "Sin coincidencias" in sensitive

    @pytest.mark.anyio
    async def test_empty_pattern_returns_error(self):
        result = await search_files.run(pattern="  ")
        assert "[ERROR]" in result

    @pytest.mark.anyio
    async def test_skip_binary_directories(self, tmp_path):
...
class TestWebSearch:
    ...
    @patch("src.tools.web_search.httpx")
    @pytest.mark.anyio
    async def test_empty_query_returns_error(self, mock_httpx):
        result = await web_search.run(query="")
        assert "[ERROR]" in result
    ...
    async def test_no_results(self, mock_httpx):
        ...
        result = await web_search.run(query="nonexistent")
        assert "No results found" in result

    @patch("src.tools.web_search.httpx")
    @pytest.mark.anyio
    async def test_network_error_returns_error(self, mock_httpx):
        mock_client = MagicMock()
        mock_client.get = AsyncMock(side_effect=httpx.ConnectError("Connection refused"))
        mock_httpx.AsyncClient.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        result = await web_search.run(query="failing query")
        assert "Search error" in result or "[ERROR]" in result

    @patch("src.tools.web_search.httpx")
    @pytest.mark.anyio
    async def test_suggestions_included(self, mock_httpx):
        mock_response = MagicMock()
        mock_response.json.return_value = MOCK_SEARCH_RESPONSE
        mock_response.raise_for_status.return_value = None
        mock_client = MagicMock()
...
class TestFetchUrl:
    ...
    async def test_fetches_url(self, mock_httpx):
        ...
        result = await fetch_url.run(url="https://example.com")
    ...
    @patch("src.tools.fetch_url.httpx")
    @pytest.mark.anyio
    async def test_empty_url_returns_error(self, mock_httpx):
        result = await fetch_url.run(url="")
        assert "[ERROR]" in result

    @patch("src.tools.fetch_url.httpx")
    @pytest.mark.anyio
    async def test_binary_content_rejected(self, mock_httpx):
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        result = await fetch_url.run(url="https://example.com/image.png")
        assert "[ERROR]" in result
    ...
    async def test_http_error_returns_error(self, mock_httpx):
        ...
        mock_client.get = AsyncMock(return_value=mock_response)
        mock_httpx.AsyncClient.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        result = await fetch_url.run(url="https://example.com/404")
