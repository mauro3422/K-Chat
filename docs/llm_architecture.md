...
>
> **Last updated:** 2026-06-17 — Updated deps: providers.py/discovery.py/retry.py now depend on config_loader for `config` param DI; model_registry, logbus, and memory pools also expose explicit configure/reset helpers for test and shutdown control.
...
| `providers.py` | Registro de providers (dict nombre→clase) + `create_provider()` explícito; `_get_provider()` quedó como wrapper compatible |
...
| `failover.py` | `_mark_and_refresh()` — marca modelo como fallido, refresca lista verificada sin bloquear el caller |
...
providers.py (Registry)
    ├── registry de clases por nombre
    ├── register_provider("openai", OpenAIAdapter)  ← se registra al importar
    ├── create_provider(config, registry) → instancia explícita
    └── _get_provider() → wrapper compatible sobre create_provider()
```
...
- **Lifecycle control**: logbus, model_registry, connection_pool y memory_pool exponen configure/reset explícitos para shutdown/tests.

## 6. Lo que podría mejorar

- **`api_call.py` concentra la llamada al proveedor** y `model_state.py` concentra el estado compartido.
- **`_MAX_RETRIES` podría aprovecharse más**: el backoff existe, pero sigue siendo conservador.
- **Duplicación de lógica de failover**: `_try_stream()` y `_with_fallback()` repiten el mismo patrón de catch→mark→switch→retry.
...
- **Lifecycle**: los resets explícitos ya existen, pero siguen existiendo entry points globales por compatibilidad.
