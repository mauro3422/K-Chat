...
async def _append_telemetry(connection: Request | WebSocket, session_id: str | None, event: dict) -> None:
    if not session_id:
        return
    try:
        repos = getattr(connection.app.state, 'repos', None) or get_repos()
