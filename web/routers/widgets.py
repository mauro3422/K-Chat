...
from src.api.tools import sanitize_widget_id
...
async def set_widget_state(session_id: str, widget_id: str, payload: WidgetStatePayload, *, request: Request = None) -> dict[str, str]:
    repos = getattr(getattr(request, "app", None), "state", None)
    repos = getattr(repos, "repos", None) or get_repos()
...
async def get_all_widget_states(session_id: str, *, request: Request = None) -> dict[str, str]:
    repos = getattr(getattr(request, "app", None), "state", None)
    repos = getattr(repos, "repos", None) or get_repos()
    await repos.sessions.require_session(session_id)
...
async def get_widget_code(session_id: str, widget_id: str, *, request: Request = None) -> Any:
    repos = getattr(getattr(request, "app", None), "state", None)
    repos = getattr(repos, "repos", None) or get_repos()
    await repos.sessions.require_session(session_id)
...
async def get_widget_versions(session_id: str, widget_id: str, *, request: Request = None) -> dict[str, Any]:
    repos = getattr(getattr(request, "app", None), "state", None)
    repos = getattr(repos, "repos", None) or get_repos()
    await repos.sessions.require_session(session_id)
...
async def get_widget_version_code(session_id: str, widget_id: str, version: int, *, request: Request = None) -> Any:
    repos = getattr(getattr(request, "app", None), "state", None)
    repos = getattr(repos, "repos", None) or get_repos()
    await repos.sessions.require_session(session_id)
...
async def save_widget(session_id: str, widget_id: str, payload: SaveWidgetPayload, *, request: Request = None) -> dict[str, Any]:
    repos = getattr(getattr(request, "app", None), "state", None)
    repos = getattr(repos, "repos", None) or get_repos()
    await repos.sessions.require_session(session_id)
